...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--lifespan-sec', type=float, required=True)
    parser.add_argument('--durability', default='transient_local')
    parser.add_argument('--seed', default='lifespan-alive')
...
class LifespanPublisher(Node):
    def __init__(self, args: argparse.Namespace) -> None:
        super().__init__('qos_lifespan_pub', start_parameter_services=False, enable_rosout=False)
        self.args = args
        ...
        qos = QoSProfile(
            ...
            lifespan=Duration(seconds=args.lifespan_sec),
        )
        ...
        self.get_logger().info(
            'publisher ready: '
            f'topic={args.topic}, durability={args.durability}, lifespan={args.lifespan_sec}s'
        )

    def _observe(self) -> None:
        current_count = self.publisher.get_subscription_count()
        self.get_logger().info(
            f'subscriber_count observation: {current_count}'
        )
